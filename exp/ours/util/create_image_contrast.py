            ...
            #web_class_to_search = np.random.choice(coco_cat_to_web[coco_class],1)
                for web_img in tqdm(web_cat_to_image_id[web_c]):
                    ...
                    for j,img in enumerate(other_images):
                        ...
                        entry['gpv_id'] = f"img-contrast-{str(coco_class)}-{str(web_c)}-{str(web_img)}-{str(img)}"
                    ...
                    contrast_group += 1
    io.dump_json_object(image_contrast_data,'/data/michal5/image_contrast/train_large.json')
