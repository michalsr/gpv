...
from exp.ours.train.new_lr_scheduler import ReduceLROnPlateau, StepLR
...
class Trainer(FromParams):
  ...
  def _get_optimizers(self, model: GPVModel, epoch_size: int, train_state: _TrainingState):
    ...
    if self.epoch_scheduler is not None:
      epoch_scheduler = StepLR(optimizer,step_size=3,gamma=0.95)
      #epoch_scheduler = ReduceLROnPlateau(optimizer, mode='max', facto r=0.1, patience=10, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08, verbose=False)
      if train_state.epoch_scheduler_state is not None:
        epoch_scheduler.load_state_dict(train_state.epoch_scheduler_state)
        new_learning_rates = epoch_scheduler.get_closed_form_lr(self.actual_epoch)
        for param_group,lr in zip(optimizer.param_groups,new_learning_rates):
          param_group['lr'] = lr
  ...
  def _train_worker(self, model, training_examples,
    ...
    for epoch in range(train_state.epoch, self.epochs):
      ...
      for i,batch in enumerate(pbar):
          ...
          if self.batch_eval == True:
            if i!=0 and i%100 == 0:
              ...
              for k in results:
                logging.info(f'Evaluation score for {k} is {results[k]} for step {i}')
                summary_writer.add_scalar(f"Intermediate Validation for {k}",results[k],global_step)
      ...
      if run_dir is not None and self.checkpoint:
        if float(self.val_score) > float(self.global_best_val):
          if not os.path.exists(self.prefix+'/global_best_model/'):
            os.mkdir(self.prefix+'/global_best_model')
          if not os.path.exists(self.prefix+'/global_best_model/r0'):
            os.mkdir(self.prefix+'/global_best_model/r0')
          torch.save(_base_model.state_dict(), self.prefix+'/global_best_model/r0/best-state.pth')
          self.global_best_val = float(self.val_score)
        ...
        if not os.path.exists(self.prefix+'/best_model/r0/'):
          os.mkdir(self.prefix+'/best_model/r0')
        torch.save(_base_model.state_dict(), self.prefix+'/best_model/r0/best-state.pth')
